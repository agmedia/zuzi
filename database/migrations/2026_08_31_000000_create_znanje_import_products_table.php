...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('znanje_import_feed_rows')) {
            Schema::create('znanje_import_feed_rows', function (Blueprint $table) {
                ...
                $table->string('source_category', 191)->nullable();
                $table->json('source_categories')->nullable();
                $table->string('source_publisher', 191)->nullable();
                $table->string('source_url', 1024);
                $table->string('image_url', 1024)->nullable();
                $table->json('additional_image_urls')->nullable();
                $table->decimal('price_eur', 15, 4)->default(0);
                $table->decimal('sale_price_eur', 15, 4)->nullable();
                ...
                $table->unique(['feed_token', 'external_id'], 'znanje_feed_rows_token_external_unique');
                $table->index(['feed_token', 'remote_product_id'], 'znanje_feed_rows_token_remote_index');
                $table->index('created_at', 'znanje_feed_rows_created_at_index');
            });
        }

        if (! Schema::hasTable('znanje_import_products')) {
            Schema::create('znanje_import_products', function (Blueprint $table) {
                $table->bigIncrements('id');
                ...
                $table->string('name');
                $table->mediumText('description')->nullable();
                $table->string('source_category', 191)->nullable()->index();
                $table->json('source_categories')->nullable();
    ...
    {
        Schema::dropIfExists('znanje_import_products');
        Schema::dropIfExists('znanje_import_feed_rows');
    }
};
