...
class ApiController extends Controller
{
    ...
    public function pelionTest(Request $request)
    {
        $data = $request->validate([
            'action' => 'required|string|in:item-list,item-list-attrs,item-by-id,group-items,group-active,item-type,item-groups,stock-list,stock-by-item,sync-item-index,sync-product-isbns,sync-product-quantities,sync-product-shelves,sync-products-from-new-sku-file,stock-by-isbn',
        ...
        if ($data['action'] === 'sync-products-from-new-sku-file') {
            return $this->syncProductsFromNewSkuFile();
        }
    ...
    private function syncProductsFromNewSkuFile()
    {
        if (
        ...
        if (! is_file($excelPath)) {
            return response()->json([
                'error' => 'Nedostaje Excel datoteka: public/USPOREDBA_kasa_novi_SKU.xlsx'
            ], 422);
        }
        ...
        try {
            ...
            $reader->setLoadSheetsOnly(['Usporedba']);

            $spreadsheet = $reader->load($excelPath);
            $sheet = $spreadsheet->getSheetByName('Usporedba') ?: $spreadsheet->getActiveSheet();
            ...
            for ($col = 1; $col <= $highestColumnIndex; $col++) {
                $header = $this->excelCellString($sheet->getCell([$col, 1])->getValue());
            ...
            $barcodeHeader = $headers['Barkod'] ?? ($headers['Berkod'] ?? null);

            foreach (['CARTIKL', 'NOVI SKU'] as $header) {
                if (! isset($headers[$header])) {
                    return response()->json([
                        'error' => 'Nedostaje Excel stupac: ' . $header,
                    ], 422);
                }
            }
            ...
            if (! $barcodeHeader) {
                return response()->json([
                    'error' => 'Nedostaje Excel stupac: Barkod/Berkod',
                ], 422);
            }

            $confidenceHeader = $headers['Pouzdanost'] ?? null;
            $scoreHeader = $headers['Score'] ?? null;
            $excelBySku = [];
            $skippedMissingSku = 0;
            $skippedMissingItemid = 0;

            for ($row = 2; $row <= $highestRow; $row++) {
                $skuRaw = $this->excelCellString($sheet->getCell([$headers['NOVI SKU'], $row])->getValue());
                $skuKey = $this->normalizeImportSkuKey($skuRaw);
                $itemid = $this->normalizeExcelItemid($sheet->getCell([$headers['CARTIKL'], $row])->getValue());
                $isbn = $this->normalizeBarcode($sheet->getCell([$barcodeHeader, $row])->getValue());
                $confidence = $confidenceHeader ? $this->excelCellString($sheet->getCell([$confidenceHeader, $row])->getValue()) : '';
                $score = $scoreHeader ? $this->excelNumericValue($sheet->getCell([$scoreHeader, $row])->getValue()) : 0.0;
                ...
                $excelBySku[$skuKey]['skus'][$skuRaw] = $skuRaw;
                $excelBySku[$skuKey]['candidates'][] = [
                    'row' => $row,
                    'sku_key' => $skuKey,
                    'excel_sku' => $skuRaw,
                    'itemid' => $itemid,
                ...
                if (count($excelBySku[$skuKey]['rows']) < 10) {
                    $excelBySku[$skuKey]['rows'][] = $row;
                }
            }

            $selectedExcelBySku = [];
            $excelDuplicateNewSku = 0;
            $excelDuplicateNewSkuItemidConflicts = 0;
            $examples = [
                'updated' => [],
                'excel_duplicate_new_sku' => [],
                'duplicate_product_sku' => [],
                'duplicate_target_itemid' => [],
            ];

            foreach ($excelBySku as $skuKey => $entry) {
                $candidates = $entry['candidates'];

                if (count($candidates) > 1) {
                    $excelDuplicateNewSku++;
                    $itemids = array_values(array_unique(array_map(fn ($candidate) => (int) $candidate['itemid'], $candidates)));

                    if (count($itemids) > 1) {
                        $excelDuplicateNewSkuItemidConflicts++;
                    }

                    if (count($examples['excel_duplicate_new_sku']) < 10) {
                        $examples['excel_duplicate_new_sku'][] = [
                            'sku_key' => $skuKey,
                            'excel_skus' => array_values($entry['skus']),
                            'chosen' => $this->chooseImportCandidate($candidates),
                ...
                $selectedExcelBySku[$skuKey] = [
                    'sku_key' => $skuKey,
                    'excel_skus' => array_values($entry['skus']),
                    'itemid' => (int) $chosen['itemid'],
                    'isbn' => $chosen['isbn'],
            ...
            $productsBySku = [];
            $productsBefore = 0;
            $productsSkippedMissingSku = 0;

            DB::table('products')
              ->select('id', 'sku', 'name', 'isbn', 'itemid')
              ->orderBy('id')
              ->chunkById(1000, function ($products) use (&$productsBySku, &$productsBefore, &$productsSkippedMissingSku) {
                  foreach ($products as $product) {
                      $productsBefore++;
                      $skuKey = $this->normalizeImportSkuKey($product->sku);

                      if (! $skuKey) {
                          $productsSkippedMissingSku++;
                          continue;
                      }
            ...
            $updates = [];
            $skippedNoProduct = 0;
            $skippedDuplicateProductSku = 0;

            foreach ($selectedExcelBySku as $skuKey => $excel) {
                if (! isset($productsBySku[$skuKey])) {
                    $skippedNoProduct++;
                    continue;
                }

                if (count($productsBySku[$skuKey]) !== 1) {
                    $skippedDuplicateProductSku++;

                    if (count($examples['duplicate_product_sku']) < 10) {
                        $examples['duplicate_product_sku'][] = [
                            'sku_key' => $skuKey,
                            'excel_skus' => $excel['excel_skus'],
                            'product_ids' => array_map(fn ($product) => (int) $product->id, $productsBySku[$skuKey]),
                        ];
                    }
                ...
                $product = $productsBySku[$skuKey][0];

                $updates[(int) $product->id] = [
                    'product_id' => (int) $product->id,
                    'sku' => $product->sku,
                    'name' => $product->name,
                    'old_isbn' => $product->isbn,
                    'new_isbn' => $excel['isbn'],
                    'old_itemid' => $product->itemid === null ? null : (int) $product->itemid,
                    'new_itemid' => (int) $excel['itemid'],
            ...
            foreach ($updates as $productId => $update) {
                $targetItemids[$update['new_itemid']][] = $productId;
            }

            $skippedDuplicateTargetItemid = 0;

            foreach ($targetItemids as $itemid => $productIds) {
                ...
                foreach ($productIds as $productId) {
                    unset($updates[$productId]);
                    $skippedDuplicateTargetItemid++;
                }

                if (count($examples['duplicate_target_itemid']) < 10) {
                    $examples['duplicate_target_itemid'][] = [
                        'itemid' => (int) $itemid,
            ...
            if ($updatesToApply) {
                ...
                foreach (array_chunk($targetProductIds, 1000) as $chunk) {
                    DB::table('products')
                        ...
                        ->orderBy('id')
                        ->get()
                        ->each(function ($product) use (&$backupRowsById) {
                            $backupRowsById[(int) $product->id] = [
                                'id' => (int) $product->id,
                                'sku' => $product->sku,
                                'name' => $product->name,
                                'isbn' => $product->isbn,
                                'itemid' => $product->itemid === null ? null : (int) $product->itemid,
                            ];
                        });
                }
                ...
                ksort($backupRowsById);
                $backupRows = array_values($backupRowsById);
                ...
                DB::transaction(function () use ($updatesToApply, $incomingItemids, &$freedItemidRows, &$updatedRows, &$examples): void {
                    ...
                    foreach (array_chunk($incomingItemids, 1000) as $chunk) {
                        $freedItemidRows += DB::table('products')
                            ->whereIn('itemid', $chunk)
                            ->update([
                                'itemid' => null,
                                'updated_at' => $now,
                    ...
                    foreach ($updatesToApply as $productId => $update) {
                        $updatedRows += DB::table('products')
                            ->where('id', $productId)
                            ->update([
                                'isbn' => $update['new_isbn'],
                                'itemid' => $update['new_itemid'],
                                'updated_at' => $now,
                            ]);

                        if (count($examples['updated']) < 10) {
                            $examples['updated'][] = $update;
                        }
                    }
                });
            }
            ...
            return response()->json([
                ...
                'url' => 'local:' . $excelPath,
                'body' => [
                    'message' => 'Privremeni import iz USPOREDBA_kasa_novi_SKU.xlsx je završen.',
                    'excel_rows' => max(0, $highestRow - 1),
                    'excel_distinct_new_sku' => count($excelBySku),
                    'excel_selected_distinct_new_sku' => count($selectedExcelBySku),
                    'excel_duplicate_new_sku' => $excelDuplicateNewSku,
                    'excel_duplicate_new_sku_itemid_conflicts' => $excelDuplicateNewSkuItemidConflicts,
                    'matched_products' => count($updates),
                    'matched_products_that_changed' => count($updatesThatChange),
                    'updated' => $updatedRows,
                    'freed_itemid_rows' => $freedItemidRows,
                    'products_before' => $productsBefore,
                    'products_with_itemid_after' => DB::table('products')->whereNotNull('itemid')->count(),
                    'products_with_isbn_after' => DB::table('products')->whereNotNull('isbn')->where('isbn', '<>', '')->count(),
                    'duplicate_non_null_itemids_after' => DB::query()
                        ->fromSub(
                            DB::table('products')
                                ->select('itemid')
                                ->whereNotNull('itemid')
                        ...
                        ->count(),
                    'backup_path' => $backupPath ? storage_path('app/' . $backupPath) : null,
                    'skipped_missing_new_sku' => $skippedMissingSku,
                    'skipped_missing_cartikl' => $skippedMissingItemid,
                    'skipped_no_product_sku_match' => $skippedNoProduct,
                    'skipped_duplicate_product_sku' => $skippedDuplicateProductSku,
                    'skipped_duplicate_target_itemid' => $skippedDuplicateTargetItemid,
                    'products_skipped_missing_sku' => $productsSkippedMissingSku,
                    'mapping' => [
                        'NOVI SKU' => 'products.sku',
                        'Barkod' => 'products.isbn',
                        'CARTIKL' => 'products.itemid',
                    ],
                    'examples' => $examples,
                ],
            ]);
        } catch (\Throwable $e) {
            Log::error('New SKU product import failed', [
                'message' => $e->getMessage(),
            ]);

            return response()->json([
                'error' => 'Greška kod importa iz USPOREDBA_kasa_novi_SKU.xlsx: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    private function excelCellString($value): string
    {
    ...
    private function normalizeImportSkuKey($value): ?string
    {
        $value = $this->excelCellString($value);
    ...
    private function normalizeExcelItemid($value): ?int
    {
        $value = $this->excelCellString($value);
    ...
    private function excelNumericValue($value): float
    {
    ...
    private function chooseImportCandidate(array $candidates): array
    {
        usort($candidates, fn ($a, $b) => $this->compareImportCandidates($a, $b));

        return $candidates[0];
    }

    private function compareImportCandidates(array $a, array $b): int
    {
        $rank = $this->importConfidenceRank($b['confidence'] ?? '') <=> $this->importConfidenceRank($a['confidence'] ?? '');
        ...
        $hasIsbn = (int) ! empty($b['isbn']) <=> (int) ! empty($a['isbn']);
    ...
    private function importConfidenceRank(string $confidence): int
    {
