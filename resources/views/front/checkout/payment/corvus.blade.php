<form name="pay" class="needs-validation w-100" action="{{ $data['action'] }}" novalidate method="POST">
    ...
    @if(!empty($data['hide_tabs']))
        <input type="hidden" name="hide_tabs" value="{{ $data['hide_tabs'] }}">
    @endif
    ...
    <div class="d-flex mt-3">
        ...
        <div class="w-50 ps-2"><button class="btn btn-primary d-block w-100" type="submit"><span class="d-none d-sm-inline">{{ !empty($data['wallet_label']) ? 'Plati uz ' . $data['wallet_label'] : 'Završite narudžbu' }}</span><span class="d-inline d-sm-none">{{ !empty($data['wallet_label']) ? $data['wallet_label'] : 'Završi kupnju' }}</span><i class="ci-arrow-right mt-sm-0 ms-1"></i></button></div>
    </div>
