<div id="checkout-livewire">
    ...
    @if ($step == 'podaci')
        ...
        <div class="d-flex pt-4 mt-3">
            ...
            <div class="w-50 ps-2"><a class="btn btn-primary d-block w-100 checkout-step-link" wire:click.prevent="changeStep('dostava')" href="{{ route('naplata', ['step' => 'dostava']) }}"><span class="d-none d-sm-inline">Na odabir dostave</span><span class="d-inline d-sm-none">Nastavi</span><i class="ci-arrow-right mt-sm-0 ms-1"></i></a></div>
        </div>
    ...
    @if ($step == 'dostava')
        ...
        <div class="table-responsive">
            <table class="table table-hover fs-sm border-top">
                <thead>
                <tr>
                    <th class="align-middle text-center" style="width: 58px;"></th>
...
@push('js_after')
    ...
    <script type="text/javascript">
        (function () {
            ...
            var checkoutScrollPending = false;
            ...
            function scrollCheckoutToTop() {
                var checkout = document.getElementById('checkout-page-start') || document.getElementById('checkout-livewire');

                if (!checkout) {
                    return;
                }

                var header = document.querySelector('.site-header.fixed-top');
                var headerOffset = header ? header.getBoundingClientRect().height : 0;
                var top = checkout.getBoundingClientRect().top + window.pageYOffset - headerOffset - 16;

                window.scrollTo(0, Math.max(0, top));
            }

            function requestCheckoutScroll() {
                checkoutScrollPending = true;
                window.setTimeout(function () {
                    if (!checkoutScrollPending) {
                        return;
                    }

                    checkoutScrollPending = false;
                    scrollCheckoutToTop();
                }, 80);
            }
            ...
            function registerLivewireHook() {
                ...
                window.Livewire.hook('message.processed', function () {
                    ...
                    if (checkoutScrollPending) {
                        checkoutScrollPending = false;
                        window.requestAnimationFrame(scrollCheckoutToTop);
                    }
                });
            }
            ...
            document.addEventListener('click', function (event) {
                if (event.target.closest('.checkout-step-link')) {
                    requestCheckoutScroll();
                }
            });
            window.addEventListener('pageshow', function () {
                if (window.location.pathname.indexOf('/naplata') !== -1) {
                    requestCheckoutScroll();
                }
            });
